...
public class Euler {
	...
	class EulerVertex {
		...
		Iterator<Graph.Edge> subTourIterator;
		...
		public EulerVertex(Graph.Vertex u) {
			...
			this.subTourIterator = subTour.iterator();
		}
	}
...
//				subTours(eulerU);
	...
	void subTours(EulerVertex eulerU) {
		while (eulerU.iterator.hasNext()) {
			...
			eulerU.subTour.add(currentEdge);
	...
	void stitchTours() {
		Graph.Vertex u = start;
			EulerVertex eulerU = getEulerVertex(u);
			while(eulerU.subTourIterator.hasNext()) {
				Graph.Edge currentEdge = eulerU.subTourIterator.next();
				tour.add(currentEdge);
				eulerU = getEulerVertex(currentEdge.otherEnd(eulerU.element));				
			}
	}
